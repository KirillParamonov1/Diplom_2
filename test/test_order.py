...
class TestOrder:
    @allure.title("Проверка создания заказа под авторизованным пользователем с ингредиентами")
    def test_order_with_login_and_ingredients(self, registered_user_payload):
        ...
        assert created_order.status_code == 200 and created_order.json()['success'] == True

    @allure.title("Проверка создания заказа под авторизованным пользователем и несуществующим хэшем ингредиентов")
    ...
    @allure.title("Проверка создания заказа под авторизованным пользователем и без ингредиентов")
    def test_order_with_login_without_ingredients(self, registered_user_payload):
        ...
        assert created_order.status_code == 400 and created_order.json()['success'] == False
    ...
    @allure.title("Проверка создания заказа без авторизации с несуществующим хэшем ингредиентов")
    ...
    def test_get_order_list_without_login_user(self):
        ...
        assert order_list.status_code == 401 and order_list.json()['success'] == False and order_list.json()['message'] == Messages.ERROR_401_AUTHORIZED_FOR_ORDER_LIST